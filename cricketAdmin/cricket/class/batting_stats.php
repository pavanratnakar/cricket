...
class Batting
{
	...
	public function getDetails($page,$limit,$sidx,$sord,$wh="")
	{
		...
		while($row = mysql_fetch_array($result,MYSQL_ASSOC)) 
		{
            ...
            $responce->rows[$i]['cell']=array($row['batting_id'],$row['firstname'],$row['lastname'],$row['team_name'],$row['match_number'],$row['four_count'],$row['six_count'],$out_status,$row['overs_played'],$row['score'],$row['overs_bowled'],$row['wide_balls'],$row['no_balls'],$row['extra_runs'],$row['runs_conceived'],$row['catches_taken']);
            $i++;
		}
		return $responce;
	}
	...
	public function getBowlingDetails($page,$limit,$sidx,$sord,$wh="")
	{
		$function=new Functions();
		$page=$function->dbCheckValues($page);
		$limit=$function->dbCheckValues($limit);
		...
		while($row = mysql_fetch_array($result,MYSQL_ASSOC)) 
		{
			...
			$responce->rows[$i]['batting_id']=$row['batting_id'];
            $responce->rows[$i]['cell']=array($row['batting_id'],$row['firstname'].' '.$row['lastname'],$row['team_name'],$row['match_number'],$row['overs_bowled'],$row['wide_balls'],$row['no_balls'],$bowling_score,$bowling_average);
            $i++;
		}
		$responce->userdata['runs_conceived'] = $bowling_total;
		return $responce;
	}
	public function getFieldingDetails($page,$limit,$sidx,$sord,$wh="")
	{
		$function=new Functions();
		$page=$function->dbCheckValues($page);
		$limit=$function->dbCheckValues($limit);
	...
	public function addDetails($four_count,$six_count,$score,$out_status,$player_id,$overs_played,$match_number,$overs_bowled,$wide_balls,$no_balls,$extra_runs,$runs_conceived,$catches_taken)
	{
		$function=new Functions();
		...
		$result=mysql_query("INSERT INTO ".BATTING."(four_count,six_count,score,out_status,player_id,overs_played,match_number,overs_bowled,wide_balls,no_balls,extra_runs,runs_conceived,catches_taken) VALUES('$four_count','$six_count','$score','$out_status','$player_id','$overs_played','$match_number','$overs_bowled','$wide_balls','$no_balls','$extra_runs','$runs_conceived','$catches_taken')");
		if($result)
		{
			return TRUE;
		}
	...
	public function editDetails($batting_id,$four_count,$six_count,$score,$out_status,$player_id,$overs_played,$match_number,$overs_bowled,$wide_balls,$no_balls,$extra_runs,$runs_conceived,$catches_taken)
	{
		$function=new Functions();
