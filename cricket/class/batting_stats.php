...
class Batting
{
	...
	public function getBattingDetails($page,$limit,$sidx,$sord,$wh="")
	{
		...
		{
			$score=$row['score'];			
			$total=$total+$score;			$six=$row['six_count'];			$total_six=$total_six+$six;			$four=$row['four_count'];			$total_four=$total_four+$four;			if($row['overs_played']==0)
			$batting_average=0;
			else
            ...
            $i++;
		}		$responce->userdata['four_count'] = $total_four;
		$responce->userdata['six_count'] = $total_six;		$responce->userdata['score'] = $total;
		return $responce;
	}
	...
	public function getBowlingDetails($page,$limit,$sidx,$sord,$fixture_id=null,$team_id=null,$wh="")
	{
		...
		while($row = mysql_fetch_array($result,MYSQL_ASSOC)) 
		{			$wickets=$row['wickets'];			$total_wickets=$total_wickets+$wickets;
            ...
            $i++;
		}		$responce->userdata['wickets'] = $total_wickets;
